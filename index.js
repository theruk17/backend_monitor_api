...
app.post("/upload", upload.single("file"), async (req, res) => {
  await readXlsxFile(req.file.buffer, { sheet: "UPDATE MNT DATA" }).then(
    (rows) => {
      //connection.connect();
      ...
      rows.forEach((row) => {
        ...
        connection.query(
          `INSERT INTO pd_monitor (mnt_id, mnt_brand, mnt_model, mnt_resolution, mnt_size, mnt_refresh_rate, mnt_price_srp, mnt_price_w_com, mnt_stock_nny, mnt_stock_ramintra, mnt_stock_bangphlat, mnt_stock_thefloat, mnt_stock_rangsit, mnt_stock_sum) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) 
  ...
  await readXlsxFile(req.file.buffer, { sheet: "KB" }).then((rows) => {
    ...
    rows.forEach((row) => {
      ...
      ON DUPLICATE KEY UPDATE kb_id = ?, kb_price_srp = ?, kb_discount = ?, kb_stock_nny = ?, kb_stock_ramintra = ?, kb_stock_bangphlat = ?, kb_stock_thefloat = ?, kb_stock_rangsit = ?, kb_stock_sum = ?`,
        [
          row[0],
          row[8],
          row[9],
...
app.delete("/admin_del_case/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM pd_case WHERE case_id = ?",
    id,
    (error, result) => {
      if (error) throw error;
      res.send("Delete Data Successsfully");
...
app.delete("/admin_del_lc/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM pd_liquid WHERE lc_id = ?",
    id,
    (error, result) => {
      if (error) throw error;
      res.send("Delete Data Successsfully");
...
app.delete("/admin_del_hs/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM pd_headset WHERE hs_id = ?",
    id,
    (error, result) => {
      if (error) throw error;
      res.send("Delete Data Successsfully");
...
app.delete("/admin_del_ch/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM pd_chair WHERE ch_id = ?",
    id,
    (error, result) => {
      if (error) throw error;
      res.send("Delete Data Successsfully");
...
app.delete("/admin_del_m/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM pd_mouse WHERE m_id = ?",
    id,
    (error, result) => {
      if (error) throw error;
      res.send("Delete Data Successsfully");
...
app.delete("/admin_del_mp/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM pd_mousepad WHERE mp_id = ?",
    id,
    (error, result) => {
      if (error) throw error;
      res.send("Delete Data Successsfully");
...
app.delete("/admin_del_mic/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM pd_mic WHERE mic_id = ?",
    id,
    (error, result) => {
      if (error) throw error;
      res.send("Delete Data Successsfully");
...
app.delete("/admin_del_s/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM pd_sink WHERE s_id = ?",
    id,
    (error, result) => {
      if (error) throw error;
      res.send("Delete Data Successsfully");
