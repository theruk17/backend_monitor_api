...
app.post('/upload', upload.single('file'), async (req, res) => {
  ...
  await readXlsxFile(req.file.buffer, { sheet: 'NOTEBOOK' }).then((rows) => {
    ...
    rows.forEach((row) => {
      ...
      [row[0], row[12], row[1], row[13], row[14]+' '+row[15], row[16], row[17], row[18], row[19], row[20], row[9],
      row[0], row[12], row[13], row[14]+' '+row[15], row[16], row[17], row[18], row[19], row[20], row[9]],
      function (err, result, fields) {
        if (err) throw err;
        console.log(`Inserted ${result.affectedRows} row(s)`)
      })
